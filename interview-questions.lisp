...
(defvar *numbers-3* (list 3 6 9 12 16 18 19 22 34 67 89 122 445 679 25347 335563 234346457234 3456535325))

;; (recursive-binary-search *numbers-3* 89)  ;=> 10

(defun recursive-binary-search (list value &optional (left 0) (right (1- (length list))))
  (unless (> left right)
    (let ((middle (floor (+ right left) 2)))
      (cond
        ((= (nth middle list) value)
         middle)
        ((> (nth middle list) value)
         (recursive-binary-search list value left (1- middle)))
        (t
         (recursive-binary-search list value (1+ middle) right))))))
